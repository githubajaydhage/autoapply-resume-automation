...
class ApplicationTracker:
    ...
    def __init__(self):
        ...
        # Current sender for multi-user support
    ...
    def _load_tracker(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(columns=[
            ...
            'match_score', 'priority', 'sender_email'
        ])
    ...
    def sync_from_sent_log(self):
        ...
        Only imports applications sent by the current sender (multi-user support).
        """
        ...
        if 'sender_email' in sent_df.columns and self.sender_email:
            sent_df = sent_df[sent_df['sender_email'].str.lower() == self.sender_email.lower()]
        ...
        for _, row in sent_df.iterrows():
            ...
            # Check if already tracked (considering sender for multi-user)
            existing = self.tracker_df[
                (self.tracker_df['hr_email'].str.lower() == email.lower())
            ]
            # If sender_email column exists, also check for same sender
            if 'sender_email' in self.tracker_df.columns and self.sender_email and not existing.empty:
                existing = existing[existing['sender_email'].str.lower() == self.sender_email.lower()]
            ...
            if existing.empty:
                ...
                new_app = {
                    ...
                    'sender_email': self.sender_email  # Track which user sent this
                }
    ...
    def get_applications_by_status(self, status: str) -> pd.DataFrame:
        ...
        df = self.tracker_df[self.tracker_df['status'] == status]
        # Filter by sender for multi-user support
    ...
    def get_pending_followups(self, days_since_apply: int = 3) -> pd.DataFrame:
        ...
        Only returns applications sent by the current sender (multi-user support).
        """
        ...
        # Filter by sender for multi-user support
        if 'sender_email' in pending.columns and self.sender_email:
            pending = pending[pending['sender_email'].str.lower() == self.sender_email.lower()]
