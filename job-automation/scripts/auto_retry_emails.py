...
class AutoRetryEmails:
    ...
    def get_failed_emails(self) -> pd.DataFrame:
        ...
        Only returns failures for emails sent by the current sender (multi-user support).
        """
        ...
        if os.path.exists(bounced_file):
            try:
                ...
                # Filter by sender if column exists
                if 'sender_email' in bounced.columns and self.sender_email:
                    bounced = bounced[bounced['sender_email'].str.lower() == self.sender_email.lower()]
        ...
        if os.path.exists(sent_log):
            try:
                ...
                if 'sender_email' in sent.columns and self.sender_email:
                    sent = sent[sent['sender_email'].str.lower() == self.sender_email.lower()]
