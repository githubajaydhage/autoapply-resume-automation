...
class SmartFollowUpSender:
    ...
    def __init__(self):
        ...
        # ⭐ Interview tracking - skip follow-ups for companies where interview already happened
        self.interview_companies_path = os.path.join(
            os.path.dirname(__file__), '..', 'data', 'interview_companies.csv'
        )
        self.interviewed_companies = self._load_interviewed_companies()
        
        # ⭐ Rejection tracking - skip follow-ups for rejected applications
        self.rejections_path = os.path.join(
            os.path.dirname(__file__), '..', 'data', 'rejections.csv'
        )
        self.rejected_companies = self._load_rejections()
    ...
    def _load_followup_log(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(columns=['recipient_email', 'company', 'sent_at', 'status', 'stage', 'sender_email'])
    
    def _load_interviewed_companies(self) -> set:
        """Load list of companies where interviews already happened - NO follow-ups for these!
        
        This prevents wasting emails on companies that are already in interview stage.
        Returns a set of (company.lower(), hr_email.lower()) tuples.
        """
        interviewed = set()
        
        if not os.path.exists(self.interview_companies_path):
            return interviewed
        
        try:
            df = pd.read_csv(self.interview_companies_path)
            
            # Check for columns (handle variations in naming)
            company_col = None
            email_col = None
            
            # Find company column
            for col in df.columns:
                if col.lower() in ['company', 'company_name', 'firm']:
                    company_col = col
                    break
            
            # Find email column
            for col in df.columns:
                if col.lower() in ['hr_email', 'email', 'recipient_email']:
                    email_col = col
                    break
            ...
            if company_col and email_col:
                for _, row in df.iterrows():
                    ...
                    # Only skip if stage contains "interview" or "interviewed" or "offer"
                    if any(keyword in stage for keyword in ['interview', 'offer', 'selected']):
                        if company and email:
                            interviewed.add((company, email))
                            logging.info(f"🎯 Skipping follow-ups for {company} (interview already scheduled)")
            
            if interviewed:
                logging.info(f"📋 Loaded {len(interviewed)} interviewed companies to skip follow-ups for")
        
        except Exception as e:
            logging.warning(f"⚠️ Could not load interviewed companies: {e}")
        
        return interviewed
    
    def _load_rejections(self) -> set:
        """Load list of companies that rejected us - NO follow-ups for these!"""
        rejected = set()
        
        if not os.path.exists(self.rejections_path):
            return rejected
        
        try:
            df = pd.read_csv(self.rejections_path)
            if 'company' in df.columns and 'hr_email' in df.columns:
                for _, row in df.iterrows():
                    ...
                    if company and email:
                        rejected.add((company, email))
                        logging.info(f"❌ Skipping follow-ups for {company} (rejected)")
            if rejected:
                logging.info(f"📋 Loaded {len(rejected)} rejected companies to skip follow-ups for")
        except Exception as e:
            logging.warning(f"⚠️ Could not load rejections: {e}")
        return rejected
    
    def _get_followup_count(self, email: str) -> int:
        """Get number of follow-ups already sent to this email by the current sender.
        
        Each user maintains their own follow-up count - User A sending to company@example.com
        doesn't affect User B's follow-up count to the same email.
        """
        if self.followup_history.empty:
            return 0
        
        # Filter by both recipient email AND sender email for multi-user support
        filter_conditions = (
            (self.followup_history['recipient_email'].str.lower() == email.lower()) &
            (self.followup_history['status'] == 'sent')
        )
        
        # If sender_email column exists, filter by current sender
        if 'sender_email' in self.followup_history.columns and self.sender_email:
            filter_conditions = filter_conditions & (
                self.followup_history['sender_email'].str.lower() == self.sender_email.lower()
            )
        
        sent = self.followup_history[filter_conditions]
        return len(sent)
    
    def _save_followup_log(self, recipient_email: str, company: str, status: str, stage: int):
        """Log sent follow-up email with stage and sender for multi-user support."""
        log_entry = {
            ...
            'status': status,
            'stage': stage,
            'sender_email': self.sender_email  # Track which user sent this follow-up
        }
    ...
    def get_contacts_to_followup(self) -> pd.DataFrame:
        ...
        IMPORTANT: Only returns contacts that were originally emailed by the current sender.
        This prevents cross-user follow-ups (e.g., Shweta won't follow up on Yogeshwari's applications).
        """
        ...
        # CRITICAL: Filter by sender_email to prevent cross-user follow-ups
        # This ensures each user only follows up on their own applications
        if 'sender_email' in df.columns and self.sender_email:
            original_count = len(df)
            df = df[df['sender_email'].str.lower() == self.sender_email.lower()]
            filtered_count = len(df)
            if original_count != filtered_count:
                logging.info(f"📧 Filtered to {filtered_count}/{original_count} emails sent by {self.sender_email}")
        else:
            logging.warning("⚠️ No sender_email column found in log - follow-ups may include other users' emails")
        ...
        for idx, row in df.iterrows():
            ...
            # ⭐⭐⭐ CRITICAL: Skip companies where interview already happened ⭐⭐⭐
            if (company, email.lower()) in self.interviewed_companies:
                logging.info(f"🎯 Skipping follow-up to {email} ({company}) - Interview already scheduled!")
                skipped_interviewed += 1
                continue
            
            # ⭐⭐⭐ CRITICAL: Skip rejected companies ⭐⭐⭐
            if (company, email.lower()) in self.rejected_companies:
                logging.info(f"❌ Skipping follow-up to {email} ({company}) - Rejected!")
                skipped_interviewed += 1
                continue
        ...
        if skipped_interviewed > 0:
            logging.info(f"🎯 Skipped {skipped_interviewed} contacts (interview already in progress)")
        return result_df
